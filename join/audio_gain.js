(() => {
  ...
  function connectVideo(video) {
    ...
    try {
      ...
      // Some browsers reject creating a second MediaElementSource for the same element.
    }
  }
  ...
  function percent() {
    return Math.round(gainValue * 100);
  }
  ...
  function buildUI() {
    ...
    wrap.innerHTML = `
      <button id="kadad-audio-gain-toggle" type="button" aria-label="تقویت صدا">
        <span id="kadad-audio-gain-label">🔊 ${percent()}%</span>
      </button>
      <div id="kadad-audio-gain-panel">
        ...
        <input id="kadad-audio-gain-slider" type="range" min="1" max="6" step="0.25" value="${gainValue}">
        <div class="kadad-audio-gain-scale"><span>100%</span><span>600%</span></div>
        <div class="kadad-audio-gain-note">بیش از ۱۰۰٪ صدای دریافتی را تقویت می‌کند.</div>
      </div>`;

    const style = document.createElement('style');
    style.textContent = `
      #kadad-audio-gain{position:fixed;left:14px;top:76px;z-index:9999;font-family:Vazirmatn,Tahoma,sans-serif;direction:rtl}
      #kadad-audio-gain-toggle{border:1px solid #31425f;background:#101d31;color:#fff;border-radius:12px;padding:9px 12px;font:800 12px Vazirmatn;cursor:pointer;box-shadow:0 8px 24px rgba(0,0,0,.25)}
      #kadad-audio-gain-panel{display:none;margin-top:8px;width:245px;padding:14px;border:1px solid #31425f;border-radius:15px;background:#0d1729;color:#fff;box-shadow:0 15px 40px rgba(0,0,0,.35)}
      ...
      #kadad-audio-gain-slider{width:100%;accent-color:#6d63ff;cursor:pointer}
      .kadad-audio-gain-scale{display:flex;justify-content:space-between;color:#91a0b8;font-size:10px;margin-top:2px}
      .kadad-audio-gain-note{color:#91a0b8;font-size:10px;line-height:1.7;margin-top:9px}
      @media(max-width:600px){#kadad-audio-gain{left:10px;top:70px}.kadad-audio-gain-panel{width:230px}}
    `;
    document.head.appendChild(style);
    document.body.appendChild(wrap);

    const toggle = document.getElementById('kadad-audio-gain-toggle');
    const panel = wrap.querySelector('#kadad-audio-gain-panel');
    const slider = document.getElementById('kadad-audio-gain-slider');
