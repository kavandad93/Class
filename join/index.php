...
(async()=>{
 ...
 function tile(id,name,local){let el=document.querySelector('[data-peer="'+CSS.escape(id)+'"]');if(!el){el=document.createElement('div');el.className='tile';el.dataset.peer=id;el.innerHTML='<div class="avatar">'+esc(initial(name))+'</div><video autoplay playsinline></video><div class="name">'+esc(name)+(local?' · شما':'')+'</div>';grid.appendChild(el)}return el}
 function attach(el,p){const v=el.querySelector('video'),tracks=[];if(p.videoTrack)tracks.push(p.videoTrack);if(p.audioTrack)tracks.push(p.audioTrack);if(tracks.length){const ids=tracks.map(t=>t.id).join('|');if(v.dataset.tracks!==ids){v.srcObject=new MediaStream(tracks);v.dataset.tracks=ids;v.play().catch(()=>{})}}el.querySelector('.avatar').style.display=p.videoTrack&&p.videoEnabled?'none':'grid'}
 function render(){if(!meeting)return;const list=[meeting.self,...meeting.participants.joined.toArray()],ids=new Set();for(const p of list){const id=String(p.id||p.userId||Math.random()),name=p.name||'کاربر';ids.add(id);attach(tile(id,name,p===meeting.self),p)}grid.querySelectorAll('[data-peer]').forEach(el=>{if(!ids.has(el.dataset.peer))el.remove()});people.innerHTML=list.map(p=>'<div class="person"><div class="person-avatar">'+esc(initial(p.name))+'</div><span>'+esc(p.name||'کاربر')+(p===meeting.self?' (شما)':'')+'</span></div>').join('');document.getElementById('count').textContent=list.length;document.getElementById('sideCount').textContent=list.length}
 ...
 const form=document.getElementById('chatForm'),input=document.getElementById('chatInput');form?.addEventListener('submit',async e=>{e.preventDefault();const msg=input.value.trim();if(!msg)return;const fd=new FormData();fd.append('class_id',classId);fd.append('message',msg);await fetch('/room/chat.php',{method:'POST',body:fd,credentials:'same-origin'});input.value=''});
 <?php endif; ?>
